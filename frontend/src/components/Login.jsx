...
function Login() {
  ...
  return (
    <div className="min-h-screen bg-transparent flex items-center justify-center p-6 relative overflow-hidden font-sans">
      ...
      <div className="absolute bottom-[-20%] right-[-20%] w-[70%] h-[70%] bg-amber-500/10 rounded-full blur-[120px]" />
      <div className="w-full max-w-[460px] z-10 animate-fade-in-up">
        <div className="glass-panel p-12 py-14 rounded-[40px] border border-white/10 shadow-[0_40px_80px_rgba(0,0,0,0.8)] relative overflow-hidden">
          <div className="text-center mb-12 relative z-10">
            <div className="inline-flex items-center justify-center w-24 h-24 rounded-[32px] bg-amber-500/10 border border-amber-500/20 mb-8 group transition-all hover:scale-105">
            ...
            <h1 className="text-4xl font-black text-white tracking-tighter uppercase italic leading-none">
              AMBER<span className="text-amber-500">-India</span>
            </h1>
            <div className="flex items-center justify-center gap-2 mt-5">
              <div className="w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse" />
              <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.45em]">Law Enforcement Portal</p>
            </div>
          </div>

          <div className="flex items-center justify-center gap-2 mb-8 relative z-10">
            <Activity size={10} className={`${isAuthenticating ? 'text-amber-400 animate-pulse' : 'text-slate-700'}`} />
          ...
          {error && (
            <div className="bg-rose-500/10 border border-rose-500/20 text-rose-400 p-4 rounded-2xl text-sm mb-8 text-center leading-relaxed relative z-10">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="space-y-5 relative z-10">
            <div className="space-y-4">
              <div className="relative group">
                ...
                <input type="text" placeholder="Username" required
                  className="w-full bg-black/40 border border-white/5 rounded-2xl p-5 pl-14 text-white placeholder-slate-600 focus:border-amber-500/50 focus:ring-1 focus:ring-amber-400/20 outline-none transition-all text-sm"
            ...
            <button type="submit" disabled={isAuthenticating}
              className="w-full bg-amber-500 hover:bg-amber-600 text-black py-5 rounded-2xl shadow-2xl flex items-center justify-center gap-3 group active:scale-[0.98] transition-all disabled:opacity-60 disabled:cursor-not-allowed">
              {isAuthenticating ? (
                <div className="flex items-center gap-3">
                  <div className="w-4 h-4 border-2 border-black border-t-transparent rounded-full animate-spin" />
                  <span className="text-sm font-black">Signing In...</span>
                </div>
              ) : (
                <div className="flex items-center gap-3">
                  <ShieldAlert size={18} />
          ...
          <div className="mt-14 flex justify-between items-center text-[9px] font-black text-slate-700 uppercase tracking-widest border-t border-white/5 pt-8 relative z-10">
            <span className="flex items-center gap-2"><Globe size={13} className="text-slate-800" /> Secure Connection</span>
            <span className="opacity-50 tracking-tighter font-mono">AMBER v2.0</span>
          </div>
        </div>
      </div>
    </div>
