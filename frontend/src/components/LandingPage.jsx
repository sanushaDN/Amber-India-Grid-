    ...
    <div className="min-h-screen bg-[#030712] text-slate-100 flex flex-col relative overflow-x-hidden grid-bg">
      ...
      <nav className="w-full flex justify-between items-center px-8 py-6 z-50 border-b border-slate-900 bg-slate-950/80 backdrop-blur-md sticky top-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black tracking-tight text-white">AMBER-<span className="text-blue-500">India</span></h1>
            <p className="text-[9px] font-black tracking-[0.2em] text-slate-500 uppercase">National Missing Persons Portal</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
           <button onClick={() => navigate('/login')} className="flex items-center gap-2 px-5 h-10 rounded-xl bg-slate-800 hover:bg-slate-700 transition-all font-black text-[10px] uppercase tracking-widest text-slate-300">
      ...
      <main className="flex-grow flex flex-col items-center text-center px-4 z-10 pt-20 pb-12">
        {/* Hero */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-900 border border-slate-800 text-blue-400 text-[10px] font-black uppercase tracking-widest mb-8 animate-slide-up shadow-lg">
          <Activity size={14} className="animate-pulse" />
          Real-time Grid • Available 24/7
        </div>
        
        <h2 className="text-5xl md:text-8xl font-black tracking-tighter mb-8 max-w-5xl animate-slide-up leading-[0.9] text-white">
          Reuniting Families Through <br />
          <span className="text-gradient-blue italic">Intelligence.</span>
        </h2>
        
        <p className="text-lg md:text-xl text-slate-400 max-w-2xl mb-12 font-medium leading-relaxed animate-slide-up" style={{ animationDelay: '100ms' }}>
        ...
        <div className="flex flex-col sm:flex-row gap-6 animate-slide-up" style={{ animationDelay: '200ms' }}>
          <button onClick={() => navigate('/report')}
            className="group px-10 py-5 bg-gradient-premium text-white font-black text-xs uppercase tracking-[0.2em] rounded-2xl flex items-center justify-center gap-4 transition-all hover:scale-105 shadow-xl shadow-blue-500/20">
            <ScanFace size={20} />
            Report a Sighting
          ...
          <button onClick={() => document.getElementById('records-section').scrollIntoView({ behavior: 'smooth' })}
            className="px-10 py-5 bg-slate-900 border border-slate-800 text-slate-300 font-black text-xs uppercase tracking-[0.2em] rounded-2xl transition-all hover:bg-slate-800">
            Browse Records
          </button>
        </div>
        ...
        <div id="records-section" className="w-full max-w-6xl mt-32 text-left animate-slide-up" style={{ animationDelay: '300ms' }}>
          <div className="flex items-end justify-between mb-10">
            <div>
              <h3 className="text-3xl font-black tracking-tight text-white">Active Search Records</h3>
              <p className="text-slate-500 text-[11px] font-black uppercase tracking-widest mt-2">Public awareness is the first line of defence</p>
            </div>
          ...
          {loadingPersons ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-slate-400 border border-slate-800 rounded-2xl bg-slate-900/50">
              ...
              <p className="font-bold text-sm text-white">No active missing person cases at this time.</p>
                ...
                return (
                  <div key={person.id} className="group rounded-2xl bg-slate-900/40 border border-slate-800 hover:border-blue-500/50 hover:shadow-2xl hover:shadow-blue-500/5 transition-all overflow-hidden flex flex-col">
                    <div className="relative h-48 overflow-hidden bg-slate-950">
                      ...
                      {hrs > 24 && (
                        <div className="absolute top-2 left-2 bg-rose-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider">URGENT</div>
                      )}
                    ...
                    <div className="p-3 flex flex-col gap-2 flex-1">
                      ...
                      <div className="flex gap-2 mt-auto pt-2 border-t border-slate-800/60">
                        <button onClick={() => navigate(`/report?personId=${person.id}`)}
                          className="flex-1 py-2 bg-blue-600/10 hover:bg-blue-600/20 border border-blue-500/30 text-blue-400 font-bold rounded-lg transition-all text-[11px] uppercase tracking-wider">
                          Report Sighting
                        </button>
          ...
          <div className="bg-slate-900/50 rounded-2xl p-1 border border-slate-800 shadow-lg">
            <div className="flex flex-col md:flex-row items-center gap-8 p-8 py-10">
              <div className="flex-1 text-left">
                ...
                <h3 className="text-3xl font-bold mb-4 text-white">Enable Neighbourhood Alerts.</h3>
                ...
                <div className="flex items-center gap-4">
                  <button onClick={toggleSubscription}
                    className={`flex items-center gap-3 px-6 py-3 rounded-xl font-bold transition-all ${subscribed ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' : 'bg-slate-800 hover:bg-slate-700 text-slate-100 border border-slate-700 active:scale-95'}`}>
                    {subscribed ? <ShieldCheck size={20} /> : <Bell size={20} />}
                    {subscribed ? 'Alerts Enabled ✓' : 'Enable Alerts'}
                  </button>
                  {!subscribed && <span className="text-[10px] text-slate-500 font-bold">Click above to allow notifications</span>}
                </div>
              </div>
              <div className="w-full md:w-[320px] h-[160px] rounded-2xl bg-slate-950 border border-slate-800 relative overflow-hidden flex flex-col items-center justify-center p-6 text-center group cursor-pointer" onClick={triggerDemo}>
                ...
                <div className="relative z-10 flex flex-col items-center gap-3">
                  ...
                  <p className="text-xs font-bold text-slate-200">Preview a Sample Alert</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-6xl mt-24 animate-slide-up" style={{ animationDelay: '400ms' }}>
          <div className="flex flex-col items-center p-10 rounded-[32px] bg-slate-900/40 border border-slate-800 hover-lift shadow-2xl group">
            ...
            <h3 className="text-xl font-black tracking-tight mb-4 text-white">AI Biometrics</h3>
            <p className="text-slate-400 text-[13px] font-medium leading-relaxed text-center">Proprietary facial landmark algorithms compare sightings against records in milliseconds.</p>
          </div>
          <div className="flex flex-col items-center p-10 rounded-[32px] bg-slate-900/40 border border-slate-800 hover-lift shadow-2xl group">
            ...
            <h3 className="text-xl font-black tracking-tight mb-4 text-white">National Grid</h3>
            <p className="text-slate-400 text-[13px] font-medium leading-relaxed text-center">A unified recovery network connecting citizens and law enforcement across all Indian states.</p>
          </div>
          <div className="flex flex-col items-center p-10 rounded-[32px] bg-slate-900/40 border border-slate-800 hover-lift shadow-2xl group">
            ...
            <h3 className="text-xl font-black tracking-tight mb-4 text-white">Secure Privacy</h3>
            <p className="text-slate-400 text-[13px] font-medium leading-relaxed text-center">Citizen reports are encrypted and accessible only by authorised officers via JWT-secured portals.</p>
          </div>
        </div>
      </main>
